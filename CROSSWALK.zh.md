# CROSSWALK（中文）— World Model × VLM × VLA × VLN

机器人 / ODM 栈的短对照。不编造引用。英文原文见 [`CROSSWALK.md`](CROSSWALK.md)。
...
- **交叉论文（intersection）**：显式把 WM 想象接到 VLA/VLN，或在语言条件控制 / 导航里使用 WM 式预测（本包中标 `intersection` 或多标签）。
...
- **动作 / 导航头**：
  ...
  - 导航 → VLN 策略 / 在地图上的 LLM tool-calling *或* 基于 WM 的预测导航（如 X-MOBILITY 一类）。
- **典型栈草图**：传感器 → VLM/编码器 →（可选：WM 做 what-if 展开）→ VLA/VLN 策略或 MPC → 底层控制器。
- **数据路径**：大规模轨迹语料（Open X-Embodiment）训 VLA；视频/交互数据训 WM；VLN 用指令—路径数据（R2R / RxR / VLN-CE）。

## 开源版图缺口（来自 `oss_repos.md` 的诚实说明）
...
- **UniSim**：未找到可交互的 UniSim 世界模型开源代码（仅研究页；`google/unisim` 无关）。
- **GameNGen**：**无核验官方 OSS**；本调研中最接近的交互视频 WM 开源：Oasis / DIAMOND。
...
- 优先核验 LICENSE 与近期提交；若干有用仓库仍无 LICENSE 文件（OSS 表中已标注）。

已 fork 到 `topsun-bot` 的上游见 [`forks_created.md`](forks_created.md)。

## 工程接入笔记（机器人 / ODM）

- **先选策略还是模型**：现在就要可部署的语言→动作 → OpenVLA/Octo 路径；需要想象 / 离线仿真 / 样本效率 → DreamerV3 / Cosmos-predict / 扩散 WM 路径。
...
- **评测诚实**：真机或连续 VLN-CE 指标与 Atari/Minecraft WM 分数分开报；数字不能直接迁移。
- **许可**：原版 V-JEPA 为 CC-BY-NC；V-JEPA 2 为 MIT——商用 ODM 前请核对。MineRL 为 NC-SA。
...
- 论文标题表：[`PAPERS.md`](PAPERS.md) / [`papers_merged.jsonl`](papers_merged.jsonl)
- 逐篇摘要与要点：[`PAPERS_DETAIL.md`](PAPERS_DETAIL.md) / [`papers_detailed.jsonl`](papers_detailed.jsonl)
